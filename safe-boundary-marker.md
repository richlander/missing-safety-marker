# `safe` marks the unsafe boundary
...
It is important to note that the `safe` as a term is both exactly correct and exactly incorrect. Unsafe boundary methods are safe in the sense that they are required to expose a safe contract equal to compiler-mediated safe methods. They are unsafe in the sense that they can call unsafe methods and participate in operations that have the risk of touching uninitiated memory. The most compelling reason to bias to `safe` is that Rust already uses it for [safe externs](https://doc.rust-lang.org/reference/items/external-blocks.html) in much the same way as is being considered for C#.
...
This first example might be confusing. On first glance, it appears like a regular method with no unsafe blocks. It calls `Unsafe.Add`, which should be 
